...
export default async function BlogIndexPage() {
  ...
  return (
    <section className="container-padded py-14 md:py-20">
      <h1 className="hero-title text-3xl md:text-5xl font-semibold tracking-tight">Blog</h1>
      <ul className="mt-6 max-w-3xl space-y-4 text-sm md:text-base leading-relaxed">
        {posts.map((post) => (
          <li key={post.slug} className="border-b border-foreground/10 pb-4">
            ...
            {post.data.date ? (
              <div className="mt-1 text-xs text-foreground/60">{new Date(post.data.date as string).toLocaleDateString()}</div>
            ) : null}
          </li>
        ))}
      </ul>
